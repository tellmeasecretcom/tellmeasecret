                    ...
                    <td style={{ padding: '1rem 1.5rem', whiteSpace: 'nowrap', fontSize: '0.875rem' }}>
                      {secret.status === 'pending' || secret.status === 'pending_review' ? (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                          <form action={`/api/admin/approve/${secret.id}`} method="POST">
                            <button type="submit" className="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 text-xs border-0 cursor-pointer">
                              ✅ Freigeben
                            </button>
                          </form>
                          <form action={`/api/admin/reject/${secret.id}`} method="POST">
                            <button type="submit" className="bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700 text-xs border-0 cursor-pointer">
                              ❌ Ablehnen
                            </button>
                          </form>
                        </div>
                      ) : (
                        <form action={`/api/admin/reset/${secret.id}`} method="POST">
                          <button type="submit" className="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700 text-xs border-0 cursor-pointer">
                            🔄 Zurücksetzen
                          </button>
                        </form>
                      )}
